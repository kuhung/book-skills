...
- **格式要求**（参照已有文件 `notes/说服的艺术_笔记.md` 的格式）:
  ...
  - **必含"分歧与残差"章节**：这是个人划线不可被 AI 替代的核心价值。对比个人划线与社区热门划线，提炼三类内容——(a) 我划了但社区没划的（个人独特关注点）；(b) 社区热门但我不认同或存疑的；(c) 我的想法与作者结论相反的。若某类为空，明确写"无"，不得省略该章节
  - **必含"全书框架图"章节**：用 Mermaid `mindmap` 代码块呈现全书主题结构（从本笔记的 H2 主题模块提炼，而非照搬原书目录）。使用代码块而非图片：GitHub 可渲染、可版本控制、AI 可读取，并作为 Step 3 提炼技能的骨架输入
...
- **内容要求**（参照已有文件 `skills/persuasion-pitch/SKILL.md` 和 `skills/running-programmer/SKILL.md` 的格式）:
  ...
  - **末尾必含 "## Field Notes (实战修正)" 章节**：初始内容为一行占位说明（"本章节沉淀该方法论在实战中被修正的经验，随使用持续更新。"）。技能在实战中暴露的偏差（第二次残差）应回写至此，使技能随使用进化
